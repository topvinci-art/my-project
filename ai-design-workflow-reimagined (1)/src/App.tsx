              ...
              >
                <div className="relative mb-4">
                   <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm font-bold text-slate-900 uppercase tracking-widest truncate">{item.style}</span>
                  </div>
...
const Page8 = () => (
  <div className="h-full p-10 md:p-14 flex flex-col bg-white">
    <div className="flex justify-between items-start mb-12 border-b border-slate-100 pb-8">
      ...
      <div className="h-14 flex items-center px-8 rounded-full bg-pink-500 text-white font-bold uppercase tracking-[0.2em] shadow-[0_0_20px_rgba(242,74,46,0.3)]">
...
const Page9 = () => {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const galleryItems = [
    { id: 'p9-1', img: "https://raw.githubusercontent.com/topvinci-art/images/refs/heads/main/image%2011.png", group: 'left' },
    { id: 'p9-2', img: "https://raw.githubusercontent.com/topvinci-art/images/refs/heads/main/image%2014.png", group: 'left' },
    { id: 'p9-3', img: "https://raw.githubusercontent.com/topvinci-art/images/refs/heads/main/image%2012.png", group: 'right' },
    { id: 'p9-4', img: "https://raw.githubusercontent.com/topvinci-art/images/refs/heads/main/image%2013.png", group: 'right' }
  ];
  ...
  return (
    <div className="h-full p-10 md:p-14 flex flex-col bg-white">
      <div className="flex justify-between items-start mb-6">
        <div>
          <span className="text-slate-400 font-bold text-sm tracking-[0.4em] uppercase mb-4 block">Stage 04</span>
          <h2 className="text-6xl font-light text-slate-900 tracking-tight">组件拆分与质量处理</h2>
        </div>
        <Layers className="text-slate-200 w-8 h-8" />
      </div>

      <div className="grid grid-cols-2 gap-12 flex-1 pt-4 overflow-hidden">
        {/* Left Column */}
        <div className="flex flex-col">
          <div className="grid grid-cols-2 gap-4 mb-6">
            {galleryItems.filter(item => item.group === 'left').map((item) => (
              <motion.div 
                key={item.id} 
                layoutId={item.id}
                onClick={() => setSelectedId(item.id)}
                className="aspect-[4/3] bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm cursor-pointer hover:shadow-lg transition-all group relative"
              >
          ...
          <PromptBlock className="mb-8">
            <p>将图2的视觉风格应用于图1框架内容的组件</p>
            <p className="text-sm text-slate-400 mt-2 font-normal">图2:主视觉；图1:原型图</p>
          </PromptBlock>

          <div className="mt-auto bg-[#F0F7FF] border border-blue-100 rounded-xl p-6 shadow-sm">
             <h4 className="text-[10px] font-bold text-blue-400 uppercase tracking-[0.3em] mb-4 flex items-center gap-2">
               <CheckCircle2 className="w-3 h-3 text-blue-500" />
               Quality Guard Checklist
             </h4>
             <div className="grid grid-cols-2 gap-y-3 gap-x-6">
               {["透明背景无杂色", "清晰度达标", "颜色一致性", "比例无变形"].map((text, i) => (
        ...
        <div className="flex flex-col border-l border-slate-100 pl-12">
          <div className="grid grid-cols-2 gap-4 mb-6">
            {galleryItems.filter(item => item.group === 'right').map((item) => (
              <motion.div 
                key={item.id} 
                layoutId={item.id}
                onClick={() => setSelectedId(item.id)}
          ...
          <div className="text-xl font-bold text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2">
            组件拆分提示词 / ATOMIC PROMPT
          </div>

          <PromptBlock className="flex-1">
            <div className="text-lg leading-relaxed text-slate-700">
              将图中每个组件拆解为单独的 UI 资源。包含以下独立元素：{"\n"}
              1.一个无文字的大背景外框组件(左右下角玫瑰装饰保留){"\n"}
              2.一个无文字的卡片背景组件(保留框内模糊场景,不要金属外框,1:1){"\n"}
              3.一个无文字按钮(确认合成的按钮)。{"\n"}
              4.一个分割线装饰(合成数量左右的装饰){"\n"}
              5.一个关闭按钮{"\n"}
              6.两个打勾按钮(一个灰色,一个金色){"\n"}
              要求：所有组件去掉文字，保持当前设计，高细节纹理，背景统一为纯黑以便切图。
            </div>
          </PromptBlock>
        </div>
      </div>
...
const Page11 = () => {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const galleryItems = [
    { id: 'p11-1', img: "https://raw.githubusercontent.com/topvinci-art/images/main/20260505015033667.png", label: "玩法页面1" },
    { id: 'p11-2', img: "https://raw.githubusercontent.com/topvinci-art/images/main/20260505015033668.png", label: "玩法页面2" },
    { id: 'p11-3', img: "https://raw.githubusercontent.com/topvinci-art/images/main/20260505015033669.png", label: "榜单" },
    { id: 'p11-4', img: "https://raw.githubusercontent.com/topvinci-art/images/main/20260505015033665.png", label: "弹窗" },
    { id: 'p11-5', img: "https://raw.githubusercontent.com/topvinci-art/images/main/20260505015033666.png", label: "小窗" },
    { id: 'p11-6', img: "https://raw.githubusercontent.com/topvinci-art/images/main/20260505015033664.png", label: "Banner" }
  ];
        ...
        <div className="grid grid-cols-6 gap-4 mt-8">
          {galleryItems.map((item) => (
            <div key={item.id} className="flex flex-col gap-3">
              ...
              >
                <div className="absolute inset-0 bg-slate-100/30 group-hover:bg-transparent transition-colors" />
                <motion.img 
                  layoutId={`img-${item.id}`}
                  src={item.img} 
...
const Page14 = () => (
  <div className="h-full p-10 md:p-14 flex flex-col bg-white">
    <div className="flex justify-between items-start mb-12 border-b border-slate-100 pb-8">
      ...
      <Clock className="text-slate-200 w-8 h-8" />
    </div>
...
const Page15 = () => (
  <div className="h-full p-10 md:p-14 flex flex-col bg-slate-50/50">
    <header className="mb-8 flex items-center gap-6">
      <h2 className="text-5xl font-bold tracking-tight text-slate-900">Prompt模板库（持续更新）</h2>
      <div className="h-[1px] flex-1 bg-slate-200" />
      <span className="text-sm font-bold text-slate-400 tracking-widest uppercase">Ongoing Updates</span>
    </header>
    ...
    <div className="h-full p-10 md:p-16 bg-white flex flex-col font-sans relative overflow-hidden">
      ...
      <div className="grid grid-cols-12 gap-16 flex-1 overflow-hidden">
        ...
        <div className={`col-span-8 flex flex-col pt-6 ${rightImages?.length === 1 ? 'overflow-hidden' : 'overflow-y-auto'} px-2 scrollbar-hide`}>
           ...
           {rightImages ? (
             <div className={`${rightImages.length === 1 ? 'flex-1 min-h-0' : `grid ${rightImages.length > 2 ? 'grid-cols-2' : 'grid-cols-1'} gap-6`}`}>
                {rightImages.map((img, idx) => (
                  <motion.div 
                    key={idx} 
                    layoutId={`right-img-${idx}`}
                    onClick={() => setSelectedId(`right-img-${idx}`)}
                    className={`${rightImages.length === 1 ? 'h-full w-full' : 'aspect-video'} bg-slate-50 border border-slate-100 rounded-[2rem] overflow-hidden flex items-center justify-center p-4 cursor-pointer hover:shadow-xl transition-all`}
                  >
                     <motion.img 
